# Microsoft Entra Agent ID sidecar (Node.js)

This sample hosts a simple Microsoft 365 Agents SDK agent on port `3978`. The
Agents SDK obtains outbound channel tokens through the Microsoft Entra Agent ID
sidecar, so the Node.js process does not load a Blueprint credential or use
MSAL.
...
- `Connections__ServiceConnection__Settings__ClientId`, set to the same
  Blueprint application client ID as `BLUEPRINT_CLIENT_ID`
- `Connections__ServiceConnection__Settings__TenantId`, set to the same
  tenant as `BLUEPRINT_TENANT_ID`

Keep the remaining `Connections__ServiceConnection__Settings__*` and
...
`npm start` loads the Agents SDK connection settings from `.env`. The same
settings are passed explicitly to the agent container by Compose; the
Blueprint secret is passed only to the sidecar container.
...
- **Missing sidecar connection:** confirm the
  `Connections__ServiceConnection__Settings__*` and `ConnectionsMap__*`
  values from `env.TEMPLATE` are present in `.env`.
- **`ClientSecret is required`:** rebuild and confirm the connection uses
  `AuthType.EntraAuthSideCar`, not an MSAL provider.
