...
This sample shows how to embed the GitHub Copilot SDK inside a Microsoft 365 Agent. The result is **Dungeon Scribe**, a fantasy RPG note-keeper that can narrate with flair, roll dice through Copilot tools, and maintain a simple per-conversation inventory.
...
- send the user's message to Copilot and return the model response to the chat client
...
Update `appsettings.json` with your bot registration values:
...
- combine a fixed persona with tool calling for RPG-style interactions
